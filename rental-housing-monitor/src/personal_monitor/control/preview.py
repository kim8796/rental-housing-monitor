...
def _format_value(value: Scalar, field_type: FieldType) -> str:
    if field_type is FieldType.URL and isinstance(value, str):
        return _plain(_redact_url(value), limit=120)
...
def _schedule_text(value: str) -> str:
    ...
    minutely = _MINUTELY_CRON.fullmatch(value)
    if minutely is not None:
        return f"{int(minutely.group(1))}분마다"
    daily = re.fullmatch(r"([0-5]?[0-9]) ([01]?[0-9]|2[0-3]) \* \* \*", value)
    if daily is not None:
        return f"매일 {int(daily.group(2)):02d}:{int(daily.group(1)):02d}"
    weekday = re.fullmatch(r"([0-5]?[0-9]) ([01]?[0-9]|2[0-3]) \* \* 1-5", value)
    if weekday is not None:
        return f"평일 {int(weekday.group(2)):02d}:{int(weekday.group(1)):02d}"
    weekly = re.fullmatch(
        r"([0-5]?[0-9]) ([01]?[0-9]|2[0-3]) \* \* ([0-7])",
        value,
    )
    if weekly is not None:
        days = ("일", "월", "화", "수", "목", "금", "토", "일")
        return (
            f"매주 {days[int(weekly.group(3))]}요일 "
            f"{int(weekly.group(2)):02d}:{int(weekly.group(1)):02d}"
        )
    monthly = re.fullmatch(
        r"([0-5]?[0-9]) ([01]?[0-9]|2[0-3]) ([1-9]|[12][0-9]|3[01]) \* \*",
        value,
    )
    if monthly is not None:
        return (
            f"매월 {int(monthly.group(3))}일 "
            f"{int(monthly.group(2)):02d}:{int(monthly.group(1)):02d}"
        )
    return f"cron: {_plain(value, limit=120)}"
