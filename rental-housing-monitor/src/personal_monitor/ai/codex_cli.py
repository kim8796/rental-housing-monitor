...
from .auth import _safe_directory, _stop_process
...
_FORBIDDEN_CAPABILITY_RE: Final = re.compile(
    r"(?:command|shell|exec|file.?change|patch|mcp|web.?search|image|tool.?call)",
    re.IGNORECASE,
)
...
def _contains_forbidden_capability(value: object) -> bool:
    stack = [value]
    while stack:
        current = stack.pop()
        ...
        elif isinstance(current, str) and _FORBIDDEN_CAPABILITY_RE.search(current):
            return True
    return False


def _validate_event(line: bytes) -> None:
    value = _bounded_json(line)
    if type(value) is not dict:
        raise CodexProtocolError
    event_type = value.get("type")
    if type(event_type) is not str or event_type not in _ALLOWED_EVENT_TYPES:
        raise CodexProtocolError
    if event_type in {"error", "turn.failed"}:
        raise CodexProtocolError
    if _contains_forbidden_capability(value):
        raise CodexProtocolError
...
async def _read_stream(stream: object, *, events: bool) -> bytes:
    ...
    if len(lines) > MAX_EVENTS or any(len(line) > MAX_LINE_BYTES for line in lines):
        raise CodexProtocolError
    if events:
        for line in lines:
            if not line:
                raise CodexProtocolError
            _validate_event(line)
    return bytes(data)
...
class CodexCli:
    ...
    def __init__(
        ...
        *,
        ...
        try:
            ...
            raise CodexProtocolError from None
        object.__setattr__(self, "_binary", resolved_binary)
        resolved_binary_metadata = os.stat(resolved_binary, follow_symlinks=False)
        object.__setattr__(
            self,
            "_binary_identity",
            (resolved_binary_metadata.st_dev, resolved_binary_metadata.st_ino),
        )
    ...
    async def run(
        ...
        try:
            ...
            _write_exclusive(request_path, request_data)
